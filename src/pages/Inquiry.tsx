...
// ✅ CHANGE: Removed "Palmistry" from Service type
...
// ✅ CHANGE: Removed Palmistry from SERVICES array
const SERVICES: { id: Service; emoji: string; label: string; desc: string }[] = [
  { id: "Astrology", emoji: "🪐", label: "Astrology", desc: "Birth chart, predictions & planetary guidance" },
  { id: "Numerology", emoji: "🔢", label: "Numerology", desc: "Life path, destiny & soul urge numbers" },
  { id: "Vastu", emoji: "🏠", label: "Vastu", desc: "Space harmonisation & energy alignment" },
...
// ✅ CHANGE: Full 1–59 range for minutes (was 5-min slabs)
const MINUTES = Array.from({ length: 59 }, (_, i) => String(i + 1).padStart(2, "0"));

const todayStr = new Date().toISOString().split("T")[0];

// ✅ CHANGE: Max date = today + 3 months
...
// ✅ CHANGE: Name validation helper — no digits/special chars, min 2 chars
...
const Step2 = ({ form, setField, errors }: { form: FormData; setField: (k: keyof FormData, v: string) => void; errors: FieldErrors }) => {
  ...
  // ✅ CHANGE: Vastu dimension options (user-friendly dropdown, 1–200 ft, no zero/negatives)
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Name */}
      {/* ✅ CHANGE: Name validation enforced — letters/spaces only, min 2 chars */}
      <Field label="Full Name" icon={<Star size={13} />} error={errors.name} required>
        <input
          type="text"
          value={form.name}
          onChange={(e) => {
            // ✅ CHANGE: For Numerology, disable manual entry — read-only handled via field note
            // For all services, still allow typing but validate on next
            setField("name", e.target.value);
          }}
          placeholder="e.g. Santosh R Pandey"
          className={iCls}
          style={iStyle(errors.name)}
          // ✅ CHANGE: Numerology — prevent copy-paste and manual entry of numbers/special chars
          onKeyDown={isNumerology ? (e) => {
            // Allow only letters, spaces, dots, hyphens, apostrophes and navigation keys
          ...
          onPaste={isNumerology ? (e) => {
            // ✅ CHANGE: Numerology — disable paste
            e.preventDefault();
          } : undefined}
        />
        {isNumerology && (
      ...
      {isAstrology && (
        <Field label="Time of Birth" icon={<Clock size={13} />}
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            {/* ✅ CHANGE: Full 1–59 range for minutes */}
            <select value={form.tob_minute} onChange={(e) => setField("tob_minute", e.target.value)}
              className={iCls} style={{ ...iStyle(errors.tob_minute), colorScheme: "dark" }}>
              <option value="">Min</option>
              {MINUTES.map((m) => <option key={m} value={m}>{m}</option>)}
      ...
      {/* ✅ CHANGE: Vastu dimensions — user-friendly dropdowns, no zero/negative values (1–200 ft) */}
      {isVastu && (
        <Field label="Property Dimensions" icon={<Ruler size={13} />}
          error={errors.length || errors.width} required hint="Select the Length and Width of your property in feet">
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1">
              ...
              >
                ...
                {DIMENSION_OPTIONS.map((v) => (
                  <option key={v} value={String(v)}>{v} ft</option>
                ))}
              </select>
            </div>
            <div className="flex flex-col gap-1">
...
const Step3 = ({ form, setField, errors }: { form: FormData; setField: (k: keyof FormData, v: string) => void; errors: FieldErrors }) => (
  <div className="space-y-5">
    ...
    {/* ✅ CHANGE: Max date restricted to today + 3 months */}
    <Field label="Preferred Date" icon={<Calendar size={13} />} error={errors.consultDate} required
      hint={form.consultDate
        ? `Selected: ${new Date(form.consultDate).toLocaleDateString("en-IN", { weekday: "long", day: "2-digit", month: "long", year: "numeric" })}`
...
const InquiryForm = () => {
  ...
  const validate = (): boolean => {
    ...
    if (step === 1) {
      // ✅ CHANGE: Stronger name validation — min 2 chars, letters only
      if (!form.name.trim()) {
      ...
      if (form.service === "Vastu") {
        // ✅ CHANGE: Vastu validation — must select from dropdown (always ≥ 1, no zero/negative possible)
