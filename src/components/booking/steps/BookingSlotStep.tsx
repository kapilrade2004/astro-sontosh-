...
// import { motion, AnimatePresence } from "framer-motion";
// import { Calendar, Clock, ArrowLeft, Info, ChevronDown, ChevronUp, IndianRupee, CheckCircle2, ChevronRight } from "lucide-react";
// import { Button } from "@/components/ui/button";
// import { Label } from "@/components/ui/label";
// import { Input } from "@/components/ui/input";
// import { Textarea } from "@/components/ui/textarea";
...
// // ── Custom Time of Birth Picker (collapsible dropdown) ───────────
...
//     // Parse 24hr value → 12hr parts for display
...
//             >
//                 <div className="flex items-center gap-2">
//                     <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${value ? "bg-primary/20" : "bg-muted/40"}`}>
//                         {value
...
//                             className="text-[10px] text-muted-foreground hover:text-primary underline underline-offset-2 transition-colors cursor-pointer px-1"
//                         >
//                             Change
//                         </span>
//                     )}
//                     <ChevronRight
//                         className={`w-4 h-4 text-primary/60 transition-transform duration-200 ${isOpen ? "rotate-90" : ""}`}
//                     />
//                 </div>
//             </button>
...
//                         className="absolute left-0 right-0 top-[calc(100%+6px)] z-50 rounded-xl border border-primary/25 bg-background/95 backdrop-blur-md shadow-2xl overflow-hidden"
//                     >
//                         {/* Hour / Minute / AM-PM selectors */}
//                         <div className="flex items-stretch divide-x divide-primary/10">

//                             {/* Hour */}
//                             <div className="flex-1 flex flex-col">
//                                 <p className="text-[8px] font-bold uppercase tracking-wider text-muted-foreground text-center pt-2 pb-1">Hour</p>
...
//                                 <p className="text-[8px] font-bold uppercase tracking-wider text-muted-foreground text-center pt-2 pb-1">Min</p>
...
//                             {/* AM / PM */}
//                             <div className="flex flex-col min-w-[56px]">
//                                 <p className="text-[8px] font-bold uppercase tracking-wider text-muted-foreground text-center pt-2 pb-1">AM/PM</p>
//                                 <div className="flex flex-col gap-1 px-2 pb-2">
//                                     {periods.map((p) => (
//                                         <button
//                                             key={p}
...
//                         {/* Preview + OK button */}
//                         <div className="flex items-center justify-between px-3 py-2.5 border-t border-primary/10 bg-primary/5">
...
//                                     </span>
//                                 ) : (
//                                     <span className="text-xs text-muted-foreground italic">Select hour above</span>
//                                 )}
//                             </div>
//                             <Button
//                                 type="button"
//                                 size="sm"
//                                 className="h-7 px-4 text-xs font-bold glow-gold"
...
//                     {/* ── Time of Birth — collapsible dropdown picker ── */}
//                     {(["astrology-exact-birth-time", "astrology-no-exact-birth-time", "astrology-in-person"].includes(bookingData.serviceId) ||
//                         bookingData.serviceId === "numerology" ||
//                         bookingData.serviceId === "premium-kundli") && (
//                         <div className="space-y-1.5">
...
// Hours 1–12, Minutes 00–59 (all 60), AM/PM
const hours   = Array.from({ length: 12 }, (_, i) => String(i + 1).padStart(2, "0"));
...
const TimeOfBirthPicker = ({ value, onChange, error }: TimeOfBirthPickerProps) => {
    ...
    // Scroll the selected minute into view when picker opens
    useEffect(() => {
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -6, scale: 0.98 }}
                        transition={{ duration: 0.18, ease: "easeOut" }}
                        className="absolute left-0 right-0 top-[calc(100%+6px)] z-50 rounded-xl border border-primary/25 bg-background/97 backdrop-blur-md shadow-2xl overflow-hidden"
                    >
                        {/* Column headers */}
                        ...
                        <div className="grid grid-cols-3 divide-x divide-primary/10" style={{ height: "180px" }}>

                            {/* Hour — 1 to 12, 4 columns of 3 */}
                                ...
                                <div className="grid grid-cols-3 gap-1">
                                    {hours.map((h) => (
                                        <button
                                            ...
                                            type="button"
                                            onClick={() => setSelectedHour(h)}
                                            className={`text-xs py-2 rounded-lg font-semibold transition-all text-center ${
                                                selectedHour === h
                                                    ? "bg-primary text-primary-foreground shadow-sm"
                                                    : "text-muted-foreground hover:bg-primary/10 hover:text-primary"
                                            }`}
                            ...
                            {/* Minute — 00 to 59, scrollable single column */}
                            <div
                                ref={minuteScrollRef}
                                ...
                                <div className="flex flex-col gap-0.5">
                                    {allMinutes.map((m) => (
                                        <button
                                            ...
                                            onClick={() => setSelectedMinute(m)}
                                            className={`text-xs py-1.5 px-2 rounded-lg font-semibold transition-all text-center w-full ${
                                                selectedMinute === m
                                                    ? "bg-primary text-primary-foreground shadow-sm"
                                                    : "text-muted-foreground hover:bg-primary/10 hover:text-primary"
                                            }`}
                                ...
                                <div className="flex flex-col gap-1">
                                    {periods.map((p) => (
                                        <button
                                            ...
                                            type="button"
                                            onClick={() => setSelectedPeriod(p)}
                                            className={`text-xs py-2.5 rounded-lg font-bold transition-all text-center ${
                                                selectedPeriod === p
                                                    ? "bg-primary text-primary-foreground shadow-sm"
                                                    : "text-muted-foreground hover:bg-primary/10 hover:text-primary"
                                            }`}
                        ...
                        <div className="flex items-center justify-between px-3 py-2.5 border-t border-primary/10 bg-primary/5">
                            <div className="flex items-center gap-2">
                                ...
                                {selectedHour ? (
                                    ...
                                    <span className="text-xs text-muted-foreground">Select hour first</span>
                                )}
                            </div>
                            <Button
                                type="button"
                                size="sm"
                                className="h-8 px-5 text-xs font-bold glow-gold"
                                disabled={!selectedHour}
                                onClick={handleConfirm}
                            >
